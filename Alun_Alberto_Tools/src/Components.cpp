...
void Mesh::debugRender() {
    ...
    if (ImGui::TreeNode("Renderer")) {
        ...
        if (ImGui::TreeNode("Material" )) {
			...
			// Diffuse color
			float dif_array[3] = { mat.diffuse.x,  mat.diffuse.y, mat.diffuse.z };
			if (ImGui::DragFloat3("Diffuse", dif_array, 0.01f, 0.0f, 1.0f)) {
				mat.diffuse = lm::vec3(dif_array[0], dif_array[1], dif_array[2]);
			}

			// Specular color
			float spe_array[3] = { mat.specular.x,  mat.specular.y, mat.specular.z };
			if (ImGui::DragFloat3("Specular", spe_array, 0.01f, 0.0f, 1.0f)) {
				mat.specular = lm::vec3(spe_array[0], spe_array[1], spe_array[2]);
			}

			// Specular gloss
			float spe_gloss = mat.specular_gloss;
			if (ImGui::DragFloat("Specular Gloss", &spe_gloss, 0.01f, 0.0f, 1.0f)) {
				mat.specular_gloss = spe_gloss;
			}

			// Ambient color
			float amb_array[3] = { mat.ambient.x, mat.ambient.y, mat.ambient.z };
			if (ImGui::DragFloat3("Ambient", amb_array, 0.01f, 0.0f, 1.0f)) {
				mat.ambient = lm::vec3(amb_array[0], amb_array[1], amb_array[2]);
			}
