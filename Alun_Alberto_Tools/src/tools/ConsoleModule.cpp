...
void ConsoleModule::BuildCommand(const char* cmd)
{
    ...
    } else  if (input.find("background") != std::string::npos)
	{
		float posx = atof(v[1].c_str());
		float posy = atof(v[2].c_str());
		float posz = atof(v[3].c_str());
		Game::get().game_instance->getGraphicsSystem().addBackgroundColor(posx, posy, posz);
		
		ConsoleWrite(false, "Executed command: '%s'\n", cmd);
	}
	else  if (input.find("debugmode") != std::string::npos)
	{
		int mode = atoi(v[1].c_str());
		Game::get().game_instance->getDebugSystem().setActive(mode);
