...
void EditorSystem::RenderNode(TransformNode& trans) {
    ...
    if (trans.children.size() > 0) {
        ImGui::Unindent(7);
        if (ImGui::TreeNode((ent.name + " " + std::to_string(trans.entity_owner)).c_str())) {
        ...
        if (ImGui::Selectable((ent.name + " " + std::to_string(trans.entity_owner)).c_str()))
        {
            selected = ent.name;
        }
    }
...
void EditorSystem::SaveSceneToFile(const std::string & scene_name)
{
    ...
    for (auto& p : ECS.entities) {
        auto mesh = ECS.getSafeComponentFromEntity<Mesh>(p.name);
...
void EditorSystem::UpdateComponentMenu(float dt)
{
    ...
    if(is_adding_component)
    {
		...
		current_item = items[indexAddComponent];
		
		ImGui::Begin("Add component", &is_adding_component);
		{
			if (ImGui::BeginCombo("Component 1", current_item))
			{
			...
			ImGui::SameLine(); 
			ImGui::AddSpace(0, 10);
			if (ImGui::Button("Add") && is_adding_component) {
				is_adding_component = false;
				int entity_id = ECS.getEntity(selected);
				switch (indexAddComponent) {

					case 0:
						if (ECS.getComponentID<Collider>(entity_id) == -1) {
					...
					case 1:
						...
						if (ECS.getComponentID<Tag>(entity_id) == -1) {
							std::cout << "Tag" << std::endl;
					...
					case 2:
						...
						if (ECS.getComponentID<Pickup>(entity_id) == -1) {
							std::cout << "Pickup" << std::endl;
							ECS.createComponentForEntity<Pickup>(entity_id);
						}
						else {
							std::cout << "Pickup already exists" << std::endl;
						}
						break;
					default:
